...
struct DCField {
    double E_x, E_y;
    DCField() : E_x(0), E_y(0) {}
    DCField(double ex, double ey) : E_x(ex), E_y(ey) {}
    Vec3 field() const { return {E_x, E_y, 0}; }
};

struct ACField {
    double E0_x, E0_y, omega, phase;
    ACField() : E0_x(0), E0_y(0), omega(0), phase(0) {}
    ACField(double e0x, double e0y, double w, double ph) : E0_x(e0x), E0_y(e0y), omega(w), phase(ph) {}
    Vec3 field_at_time(double t) const {
        return {E0_x * cos(omega * t), E0_y * cos(omega * t + phase), 0};
    }
};
...
struct SuperlatticeParams {
    ...
    double mass;
    double sound_vel;
    double deform_pot_ac;
    double deform_pot_op;
    double optical_phonon_energy;
    double density;
};
...
enum DumpFlags {
    none = 0, number = 1, time = number << 1, momentum = time << 1,
    energy = momentum << 1, velocity = energy << 1, scattering = velocity << 1,
...
struct Results {
    ...
    Results(size_t cap, DumpFlags flags = DumpFlags::none) : size(0), flags(flags), average_velocity{0,0,0} {
        ns.reserve(cap); ts.reserve(cap); momentums.reserve(cap);
        velocities.reserve(cap); energies.reserve(cap); scatterings.reserve(cap);
    }
