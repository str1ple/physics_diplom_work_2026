...
double gamma = 2 * m.params.mass * m.params.delta * m.params.period * m.params.period /
(consts::hbar * consts::hbar);
double a = gamma / (m.params.delta);
double k2 = 2.0 / (1.0 - a);
if (k2 < 0 || k2 > 1) k2 = 0.5;
double F = ellint_1(k2);
return prefactor * F;
}

double AcousticScattering::total_rate(double temperature) const {
return constant;
}

// рассеяние на акуст фононах - упругое
...
double prefactor = pow(D_op, 2) * m.params.mass /
(2 * math::pi * consts::hbar * consts::hbar * rho * omega);
if (emission && m.energy(p) < omega) {
return 0;
}
return prefactor * factor;
}

double OpticalScattering::total_rate(double temperature) const {
return constant;
}

// расс на опт фононах - неупругое
...
if (v_count > 0) {
result.average_velocity = v_sum / v_count;
}
}
